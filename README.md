...
이삿짐 생성(고객) → 견적 요청(고객) → 관할지역 견적요청 목록 확인(기사) → 특정 견적요청 상세보기(기사) → 견적서 제출(기사) → 견적서 리스트 확인(고객) → 견적서 선택(고객) → 본인 견적서 선택 확인(기사) → 이사완료 확인(고객/기사)으로 시스템상 종결 처리.

<br/>

서비스 요청자와 제공자들 간의 유기적이고 연속되는 행위들을 명확한 단계로 나눔과 동시에 연계와 연속성을 가지도록 고려하며 Database를 설계하고 각각의 api을 구현하였습니다.
...
고객은 동시에 한건 이상 견적요청을 할 수 없으며 기사는 한 건의 견적 요청에 대해 한 건의 견적서만을 제출할 수 있습니다.
...
- 24시간이 경과되었거나 이미 10건의 견적서를 받은 견적요청은 포함되지 않습니다.
...
- 접수한 메시지의 createdAt이 위에서 업데이트한 시간보다 큰 경우만 true가 되고 새 메시지라고 판단하여 확인하지 않은 메시지로써 카운팅할 수 있도록 하였습니다.
- Pagenation을 적용하였고 만약 확인하는 페이지가 1페이지가 아니라면 updatedAt을 갱신하지 않게 하였는데, 사용자가 페이지 2 이상을 체크하고 있다는 것은 페이지 1의 새로운 메시지를 확인하지 않았다는 의미이기 때문입니다.
...
- 프론트에서 고객이 접속하여 email로 room을 만들어놓았다는 가정 하에 기사가 견적서를 제출하면서 고객 이메일로 동일한 room에 입장하여 메시지를 날리고 바로 room에서 logout하는 방식입니다.
