...
export class TasksService {
    ...
    try {
      ...
      if (
        isNegoing.find(
          (v) =>
            v.MovingStatusId ===
            (MovingStatusEnum.NEGO || MovingStatusEnum.PICK),
        )
      ) {
  ...
  async submitEstimate(
    ...
    try {
      ...
      const isDuplication = await this.negotiationsRepository
        .createQueryBuilder('nego')
        .where('nego.MovingInformationId = :movingInfoId', { movingInfoId })
        .andWhere('nego.BusinessPersonId = :businessPersonId', {
          businessPersonId,
        })
        .getOne();
      ...
      const countNegoByBusinessPersons = async (
        movingInfoId,
      ): Promise<number> => {
      ...
      const checkBeforeSubmitCount = await countNegoByBusinessPersons(
        movingInfoId,
      );
      ...
      // 현재 제출된 견적서 추가 합계 확인
      const checkAfterSubmitCount = await countNegoByBusinessPersons(
        movingInfoId,
      );
      ...
      if (checkAfterSubmitCount <= 9) {
        ...
        this.eventsGateway.server.to(owner.User.email).emit('message', {
          data: `현재 ${checkAfterSubmitCount}건의 견적을 받으셨습니다!`,
        });
    ...
    try {
      ...
      for (let i = 0; i < estimateList.length; i++) {
        ...
        if (estimateList[i].BusinessPerson.Reviews.length >= 1) {
          for (
            let j = 0;
            j < estimateList[i].BusinessPerson.Reviews.length;
            j++
          ) {
            stars += estimateList[i].BusinessPerson.Reviews[j].star;
          }
          const starAvg = stars / estimateList[i].BusinessPerson.Reviews.length;
      ...
      console.log(
        'estimateList:::',
        util.inspect(estimateList, { depth: null }),
        'length:::',
        estimateList.length,
      );
  ...
  async pickEstimate(movingInfoId: number, businessPersonId: number) {
    const queryRunner = this.connection.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
  ...
  async makeMovingToDoneByUser(movingInfoId: number, businessPersonId: number) {
    const queryRunner = this.connection.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
    try {
